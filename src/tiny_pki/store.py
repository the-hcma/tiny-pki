...
class CertificateStore:
    ...
    def ensure_layout(self) -> None:
        ...
        if not self.index_path.exists():
            if self.has_ca():
                raise FileNotFoundError(f"Expected index.json under {self.root} (CA present but index missing)")
            self._write_index([])
    ...
    def read_ca(self) -> tuple[bytes, bytes]:
        ...
        if not self.has_ca():
            raise FileNotFoundError(f"Expected CA files under {self.root}")
    ...
    def list_certificates(self) -> list[IssuedCertificate]:
        """Return issued certificates that still have on-disk paths (live or revoked)."""
        return [e for e in self._read_index() if e.cert_path]
    ...
    def add_certificate(
        ...
        *,
        ...
        cert_rel = f"certs/{safe}-{serial_hex}.crt"
        key_rel = f"certs/{safe}-{serial_hex}.key"
        ...
        for existing in self._read_index():
            if (
                ...
                if existing.serial_number == serial_hex:
                    # Same path as the bytes just written — drop the old row only.
                    continue
