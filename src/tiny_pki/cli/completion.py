...
_LIST_ARGS = ("ca", "certs", "clients", "revoked", "servers")
_SHOW_ARGS = ("ca", "certs", "clients", "crl", "revoked", "servers")
...
_tiny_pki_completion() {{
  ...
  case "$prev" in
    ...
    list) COMPREPLY=( $(compgen -W "{list_args}" -- "$cur") ); return ;;
...
def _fish_script() -> str:
    ...
    for arg in _LIST_ARGS:
        lines.append(f"complete -c {_PROG} -n '__fish_seen_subcommand_from list' -a {arg}")
