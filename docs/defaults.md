...
| PKCS#12 password | at least 8 bytes (`MIN_PKCS12_PASSWORD_LENGTH`) | none | The bundle can be attacked offline. The CLI takes the password from a prompt or `TINY_PKI_P12_PASSWORD`, and warns on `--password` (visible in history and `ps`). |
| Fernet secret | at least 32 characters (`tiny_pki.secrets.MIN_SECRET_LENGTH`) | none | The key is `sha256(secret)` with no stretching, so the secret must already be high-entropy (Django's `SECRET_KEY` is 50). Decryption accepts shorter secrets so you can rotate off them. |
