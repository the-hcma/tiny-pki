...
| CN as SAN | a host-like CN missing from the SANs is added, with a warning (the CLI asks first, or add `--yes` / `--no-cn-san`); on a name-constrained CA with no subtree for the CN's type (a DNS CN on an IP-only CA, or the reverse) it is left out, with a warning | Browsers ignore the CN (RFC 6125); forgetting to repeat it in the SANs is the most common private-CA mistake. |
