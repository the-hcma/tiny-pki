...
| 8 | A CA with only IP permitted subtrees leaves DNS SANs unconstrained (RFC 5280 semantics), and non-interactive `create server` auto-adds the CN as a DNS SAN, so an IP-only CA issues e.g. `google.com` that OpenSSL accepts; `docs/security.md` overstates what `permitted_subtrees` guarantees | Medium | Grok | **Done** — verified with `openssl verify`. Once a CA has permitted subtrees, issuance refuses a DNS name without a DNS subtree and an IP without an IP subtree (library and CLI), the CN is not auto-added for an unlisted type, and `docs/security.md` / `docs/api.md` now say what relying parties do and do not enforce, in the-hcma/tiny-pki#111 | the-hcma/tiny-pki#97 (under #78) |
| 9 | A restored older (still unexpired) CRL silently un-revokes certificates: `check` trusts the CRL bytes over `index.json`, exits 0, and `--include-revoked` reports the revoked leaf as `ok`; `check_certificate` also treats a CRL past `nextUpdate` as proof of non-revocation | Medium | Grok | **Done** — verified: after rollback, `openssl verify -crl_check` accepts the revoked leaf and `tiny-pki check --within 0` exits 0. Store `check` now treats `index.json` as authoritative (revoked leaves report `revoked`, a CRL missing index revocations is `untrusted`), and a CRL past `nextUpdate` makes leaves `untrusted` (revocation status unknown), in the-hcma/tiny-pki#109 | the-hcma/tiny-pki#98 (under #78) |
| 10 | `get_certificate` matches a needle against CN **or** serial, first live match wins; a CN equal to another cert's serial makes lookups ambiguous | Low (reviewer: Medium) | Grok | **Done** — verified, but downgraded: `revoke <serial>` correctly revokes the cert with that serial; the actual hazard is the reverse (looking up the cert *named* like a serial hits the other one), and it needs a deliberately chosen 40-hex-char CN. Lookups now refuse a serial/CN collision as ambiguous (with or without `0x`), and `revoke` no longer re-resolves by bare serial, in the-hcma/tiny-pki#110 | the-hcma/tiny-pki#100 (under #78) |
| 11 | `delete --force` on an active cert removes it from the index without tombstoning it, so the regenerated CRL omits the serial and exported copies stay valid | Low | Grok | **Done** — verified: exported cert still passes `openssl verify -crl_check` after `delete --force`. `delete --force` now revokes (tombstone + CRL) before removing the files, in the-hcma/tiny-pki#110 | the-hcma/tiny-pki#100 (under #78) |
| 12 | Leaf (`add_certificate`) and bundle (`write_bundle`) writes follow a symlink whose target stays inside the store, so they can overwrite `ca/ca.key` | Low | Grok (variant of row 2) | **Done** — reproduced by the reviewer's script. Leaf and bundle writes, and leaf reads for `export`, now go through `_validated_write_path` (no symlink at any component) in the-hcma/tiny-pki#107 | the-hcma/tiny-pki#99 (under #78) |
| 13 | Store directories and `index.json` / `crl.pem` / `ca.crt` take the process umask (mode `0777` under umask `000`); only secrets get an explicit `0600` | Low | Grok | **Done** — reproduced by the reviewer's script. New store directories are `0700`, `index.json` is `0600`, and existing store directories the caller owns lose world write on open (group write and read access kept for shared stores and TLS servers) in the-hcma/tiny-pki#107 | the-hcma/tiny-pki#99 (under #78) |
| 14 | `index.json` `key_path` / `cert_path` only need to stay under the store root, so `key_path: "ca/ca.key"` makes `export` ship the CA key as the leaf key | Low | Grok | **Done** — reproduced by the reviewer's script. Index paths must be `clients/<file>` or `servers/<file>` with the matching `.crt` / `.key` suffix, never `ca.key`, including on legacy migration, in the-hcma/tiny-pki#107 | the-hcma/tiny-pki#99 (under #78) |
| 15 | `--permit .example.com` has its leading dot stripped, silently permitting the apex (OpenSSL syntax means subdomains only) | Low | Grok | **Done** — a leading dot is now rejected with an explanation (the plain suffix already covers the apex and every subdomain), in the-hcma/tiny-pki#112 | the-hcma/tiny-pki#101 (under #78) |
| 16 | An excluded IPv4 range does not cover the IPv4-mapped IPv6 form (`::ffff:10.1.2.3`) | Low | Grok | **Done** — matches a literal reading of RFC 5280; matters only for clients that normalize mapped addresses. Excluded checks now unmap `::ffff:a.b.c.d` (and mapped constraints) before comparing; permitted checks stay literal because OpenSSL compares within one address family, in the-hcma/tiny-pki#112 | the-hcma/tiny-pki#101 (under #78) |
| 17 | The CLI never persists `crlNumber` (default is microseconds since the epoch), so a backward clock step can publish a lower number than the previous CRL | Low | Grok | **Done** — the store records the last number in `ca/crlnumber` and the CLI publishes `max(now in µs, last + 1)` in the-hcma/tiny-pki#108 | the-hcma/tiny-pki#102 (under #78) |
| 18 | `ca.key`, leaf keys, and `crl.pem` are written in place with `O_TRUNC` (no temp file + `fsync` + `rename`), so a crash mid-write can leave a truncated key or CRL | Low | Grok | **Done** — every store write (CA, leaves, bundles, CRL, index) is a same-directory temp file, `fsync`, `rename`, then a directory `fsync`, in the-hcma/tiny-pki#108 | the-hcma/tiny-pki#102 (under #78) |
| 19 | `TINY_PKI_GIT_SHA` is printed by `--version` without a hex check (newlines / escapes pass through) | Low | Grok | **Done** — the env override, the embedded stamp, and `git rev-parse` output must match `[0-9a-fA-F]{7,40}`, otherwise `unknown`, in the-hcma/tiny-pki#113 | the-hcma/tiny-pki#103 (under #78) |
| 20 | Library `validity_days` has no upper bound, so huge values raise `OverflowError` instead of `TinyPkiError` (the CLI caps `--days`) | Low | Grok | **Done** — new `MAX_VALIDITY_DAYS` (36500) caps CA, leaf, and CRL `validity_days` with `TinyPkiError`; the CLI uses the same constant, in the-hcma/tiny-pki#113 | the-hcma/tiny-pki#103 (under #78) |
| 21 | CI installs `uv` with `version: latest` and `pip-audit` unpinned (in the `issues: write` CVE job); Dependabot auto-merge includes `cryptography` bumps | Low | Grok | **Done** — `setup-uv` now installs an exact `uv` release (0.12.19) in `ci.yml` and `cve-check.yml`, bumped deliberately, in the-hcma/tiny-pki#114. **Accepted by @thehcma:** `pip-audit` stays unpinned (the scanner should track new advisories) and Dependabot keeps auto-merging `cryptography` bumps (CI gates them, and staying current on the crypto backend is the safer default) | the-hcma/tiny-pki#104 (under #78) |
| 22 | Docs caveats: README library snippet sets no `permitted_subtrees`; README says `cryptography` ≥ 43.0.1 while `pyproject.toml` requires ≥ 50.0.1; the nginx `check` recipe has no CRL input; name constraints do not cover client CNs | Info | Grok | **Done** — README library CA is constrained (with a warning) and its `cryptography` floor matches `pyproject.toml` (both guarded by `test_readme`), file-mode `check` gained `--crl` and the nginx recipe says expiry and revocation are separate checks, and `docs/security.md` says constraints don't limit client identities, in the-hcma/tiny-pki#115 | the-hcma/tiny-pki#105 (under #78) |
