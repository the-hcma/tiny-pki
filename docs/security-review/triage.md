...
| 2 | `write_ca` / `write_crl` / `_write_index` follow a pre-planted symlink at the fixed `ca.key` / `ca.crt` / `crl.pem` / `index.json` paths (unlike leaf cert/key paths, which are already hardened via `_path_under_root`) | High | Claude | Fix required | follow-up issue (fix) |
