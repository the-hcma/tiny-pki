...
| 15 | `--permit .example.com` has its leading dot stripped, silently permitting the apex (OpenSSL syntax means subdomains only) | Low | Grok | **Done** — a leading dot is now rejected with an explanation (the plain suffix already covers the apex and every subdomain), in the-hcma/tiny-pki#125 | the-hcma/tiny-pki#101 (under #78) |
| 16 | An excluded IPv4 range does not cover the IPv4-mapped IPv6 form (`::ffff:10.1.2.3`) | Low | Grok | **Done** — matches a literal reading of RFC 5280; matters only for clients that normalize mapped addresses. Excluded checks now unmap `::ffff:a.b.c.d` (and mapped constraints) before comparing; permitted checks stay literal because OpenSSL compares within one address family, in the-hcma/tiny-pki#125 | the-hcma/tiny-pki#101 (under #78) |
