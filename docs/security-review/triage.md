...
| 7 | `export pem` / `export p12 --out` write through a pre-existing symlink at the destination (`_write_secret_file` opens with `O_CREAT\|O_TRUNC`, no `O_NOFOLLOW`), so a planted link in the export directory receives the leaf private key | High | Grok | **Open** — verified: a symlink at `./alice.pem` got the full cert + key PEM. Same bug class as row 2, on the one write path outside the store | the-hcma/tiny-pki#96 (under #78) |
